...
namespace posest {
    ...
    inline cv::Point3f CROSS(cv::Point3f v1, cv::Point3f v2) {
        cv::Point3f tmp_p;
    ...
    inline double DOT(cv::Point3f v1, cv::Point3f v2) {
        return v1.x * v2.x + v1.y * v2.y + v1.z * v2.z;
    }
    
    inline cv::Point3f SUB(cv::Point3f v1, cv::Point3f v2) {
        cv::Point3f tmp_p;
        tmp_p.x =  v1.x - v2.x;
        tmp_p.y =  v1.y - v2.y;
        tmp_p.z =  v1.z - v2.z;
    ...
    cv::Point3f GetNearest3DPoint(std::vector<cv::Point3f> &points_list, cv::Point3f origin) {
    ...
    void PnPProblem::Set_P_matrix( const cv::Mat &R_matrix, const cv::Mat &t_matrix) {
        ...
        P_matrix_.at<double>(0,3) = t_matrix_.at<double>(0);
        P_matrix_.at<double>(1,3) = t_matrix_.at<double>(1);
        P_matrix_.at<double>(2,3) = t_matrix_.at<double>(2);
    }
    ...
    void PnPProblem::EstimatePoseRANSAC(const std::vector<cv::Point3f> &list_points3d, // list with model 3D coordinates
                                        ...
                                        int flags, cv::Mat &inliers, int iter_count,  // PnP method; inliers container
                                        float reprojection_error, double confidence ) {    // Ransac parameters
        cv::Mat dist_coeffs = cv::Mat::zeros(4, 1, CV_64FC1);  // vector of distortion coefficients
        ...
        cv::solvePnPRansac(list_points3d, list_points2d, A_matrix_, dist_coeffs, rvec, tvec,
                           use_extrinsic_guess, iter_count, reprojection_error, confidence,
                           inliers, flags );
        
        cv::Rodrigues(rvec, R_matrix_);      // converts Rotation Vector to Matrix
    ...
    bool PnPProblem::Backproject2DPoint(const Mesh *mesh,
        ...
        for (unsigned int i = 0; i < triangles_list.size(); i++) {
            ...
            if(this->IntersectMollerTrumbore(R, T, &out)) {
                ...
                intersections_list.push_back(tmp_pt); //交点列表
            }
        }
